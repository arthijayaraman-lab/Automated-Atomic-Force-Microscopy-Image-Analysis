...
if __name__ == "__main__":
    ...
    parser.add_argument('-npcs', type=int , default=3, help='Number of components to extract in PCA')
    parser.add_argument('-hist_eql', action='store_true', help='Flag to perform histogram equalization on pre-processed image')
    parser.add_argument('-pix_to_nm', type=float , default=5.21, help='Scale to convert one pixel into real distance units (nm/pix in our case)')
    parser.add_argument('-i', type=str , default="./input/", help='Folder contain input images to process')
    parser.add_argument('-C', type=str , choices=["gmm", "k_mean"], default="k_mean", help='Choose which clustering algorithum to use')
